...
class AIService:
    ...
    async def chat_for_sensory_guide(self, input_prompt: str) -> ChatForSensoryGuide.Response:
        """
        Service 레이어: 비즈니스 로직 처리
        2단계 처리:
        1. sensory_guide label로 웹 검색 및 정보 수집
        2. 수집한 정보를 바탕으로 sensory_guide label로 구조화된 출력
        
        Args:
            input_prompt: 사용자 입력 프롬프트
        
        Returns:
            매핑된 응답 데이터 + 센서리 가이드 메시지
