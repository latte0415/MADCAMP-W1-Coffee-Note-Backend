...
from schema import ChatTest, ChatForMapping, ChatForSensoryGuide
...
@router.post("/chat-for-sensory-guide")
async def chat_for_sensory_guide(request: ChatForSensoryGuide.Request) -> ChatForSensoryGuide.Response:
    """
    API 레이어: HTTP 요청/응답 처리
    Service 레이어의 예외를 HTTPException으로 변환합니다.
    """
    try:
        result = await ai_service.chat_for_sensory_guide(request.message)
        return result
    except ServiceException as e:
        # Service 레이어 예외를 HTTP 예외로 변환 (원래 예외 정보 유지)
        raise HTTPException(status_code=500, detail=str(e)) from e
